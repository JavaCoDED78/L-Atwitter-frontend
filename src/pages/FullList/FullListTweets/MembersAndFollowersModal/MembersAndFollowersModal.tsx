...
import { fetchListFollowers, fetchListMembers, resetListMembersState } from "../../../../store/ducks/listMembers/actionCreators";
...
const MembersAndFollowersModal: FC<MembersAndFollowersModalProps> = ({ listId, listOwnerId, visible, title, onClose }): ReactElement | null => {
        ...
        <Dialog open={visible} onClose={onClose} onClick={handleClick}>
            ...
            <DialogContent className={globalClasses.dialogContent}>
                {isLoading ? (
                    ...
                    users.map((user) => <ManageMembersItem key={user.id} listId={listId} listOwnerId={listOwnerId} user={user} />)
                ) : (
                    <EmptyPageDescription
                        title={title === "List members" ? "There isn’t anyone in this List" : "There aren’t any followers of this List"}
                        subtitle={
                            title === "List members" ? "When people get added, they’ll show up here." : "When people follow, they’ll show up here."
                        }
                    />
                )}
            </DialogContent>
