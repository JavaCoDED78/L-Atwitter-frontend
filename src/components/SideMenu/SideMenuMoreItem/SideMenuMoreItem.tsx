...
import { selectUserDataFollowerRequestsSize, selectUserDataIsPrivateProfile } from "../../../store/ducks/user/selectors";
...
const SideMenuMoreItem: FC<DisplayProps> = ({ changeBackgroundColor, changeColorScheme }): ReactElement => {
    ...
    return (
        <li className={classes.itemWrapper}>
            <div id={"openPopup"} aria-describedby={popoverId} onClick={handleOpenPopup}>
                <Hidden smDown>
                    <span>{MoreIcon}</span>
            ...
            >
                <div className={classnames(classes.listItemWrapper, globalClasses.svg)}>
                    <List>
                        {isPrivateProfile && (
                            <ListItem id={"openFollowerRequestsModal"} onClick={onOpenFollowerRequestsModal}>
                                ...
                                <Typography variant={"body1"} component={"span"}>
                                    Follower requests
                                    <span className={classes.followerRequestsCount}>{followerRequestsSize}</span>
                                </Typography>
                            </ListItem>
                        )}
                        ...
                        <a href={TWITTER_LOGIN} target={"_blank"} rel={"noreferrer noopener"}>
                            <ListItem>
                                {TwitterAdsIcon}
                                <Typography variant={"body1"} component={"span"}>
                                    Twitter Ads
                        ...
                        <a href={TWITTER_ABOUT} target={"_blank"} rel={"noreferrer noopener"}>
                            <ListItem>
                                {AnalyticsIcon}
                                <Typography variant={"body1"} component={"span"}>
                                    Analytics
                        ...
                        <a href={HELP_TWITTER} target={"_blank"} rel={"noreferrer noopener"}>
                            <ListItem>
                                {HelpCenterIcon}
                                <Typography variant={"body1"} component={"span"}>
                                    Help Center
            ...
            <FollowerRequestsModal visible={visibleFollowerRequestsModal} onClose={onCloseFollowerRequestsModal} />
            <DisplayModal
