...
const LargeLinkPreview: FC<LargeLinkPreviewProps> = ({ link, linkTitle, linkDescription, linkCover, isFullTweet }): ReactElement => {
    const classes = useLargeLinkPreviewStyles({ isFullTweet: isFullTweet });
    const matches = link.match(/^https?\:\/\/([^\/?#]+)(?:[\/?#]|$)/i);
    const domain = matches && matches[1];

    return (
        <a className={classes.siteLink} target={"_blank"} rel={"noreferrer noopener"} href={link}>
